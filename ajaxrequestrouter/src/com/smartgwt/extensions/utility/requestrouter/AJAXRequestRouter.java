 ...
 * 3) [JSON] String							<-> A common String, usually carefully formatted according to
 ...
 * to handle the AJAX response; this handlers must implement AJAXRequestHandler interface (for JavaScriptObject 
...
public class AJAXRequestRouter {
	 ...
	 * @param handler A class implementing the interface AJAXRequestHandler. This class will have to handle the data retrieved and the situations where the waiting_queue is full and the request has to wait or the request fails 
	 */
	 public void addNewRequest(String url, AJAXRequestHandler handler){
		@SuppressWarnings("rawtypes")
	 ...
	 */
	public void addNewRequest(String url, AJAXRequestHandler handler, RequestPriority priority, short timeout){
		@SuppressWarnings("rawtypes")
		AJAXRequest request = new AJAXRequest(url, router, handler);
	 ...
	 * Add a new AJAX request to the waiting_queue. No timeout parameter is passed, so the default value set in AJAXRequest class is used instead;
	 * @param url The url where the request has to be sent
	 * @param handler A class implementing the interface JsArrayRequestHandler. This class will have to handle the data retrieved and the situations where the waiting_queue is full and the request has to wait or the request fails 
	 */
	 @SuppressWarnings({ "rawtypes", "unchecked" })
	 public void addNewRequest(String url, JsArrayRequestHandler handler){
		 AJAXRequest request = new AJAXRequest(url, router, handler);
		 sendNewRequest(request);
	 ...
	 */
	 @SuppressWarnings({ "rawtypes", "unchecked" })
	public void addNewRequest(String url, JsArrayRequestHandler handler, RequestPriority priority, short timeout){
		AJAXRequest request = new AJAXRequest(url, router, handler);
		request.setPriority(priority);
		request.setTimeout(timeout);		
		sendNewRequest(request);
	 ...
	 * Add a new AJAX request to the waiting_queue. No timeout parameter is passed, so the default value set in AJAXRequest class is used instead;
	 * @param url The url where the request has to be sent
	 * @param handler A class implementing the interface JSONRequestHandler. This class will have to handle the data retrieved and the situations where the waiting_queue is full and the request has to wait or the request fails 
	 */
	 public void addNewRequest(String url, JSONRequestHandler handler){
		@SuppressWarnings("rawtypes")
		AJAXRequest request = new AJAXRequest(url, router, handler);
		sendNewRequest(request);
	 ...
	 * @param timeout The maximum duration (in seconds) of the AJAX request (a default value is set in AJAXRequest class)
	 */
	public void addNewRequest(String url, JSONRequestHandler handler, RequestPriority priority, short timeout){
		@SuppressWarnings("rawtypes")
		AJAXRequest request = new AJAXRequest(url, router, handler);
		request.setPriority(priority);
		request.setTimeout(timeout);
